    ...
    public onCandidate(json: string): void{
        ...
        if(!("dahai" in parsed)){
            if("chi" in parsed || "pon" in parsed || "kan" in parsed || "hora" in parsed){
                ...
                const self = this
                this.controller.node.getChildByName("Skip Button").on("click", () => {
                    self.controller.getProtocol().emit("skip", {"protocol": "skip"}, false)
                    self.controller.node.getChildByName("Skip Button").active = false
                }, this)
            }
        }
    }
